<template>
  <div class="container">
    <header class="header">
      ...
      <button v-if="currrentUser ==='admin'" @click="addNewCourse" class="action-button add-course-button">
        <i  class="icon icon-add"></i> Add New Course
      </button>
      <router-link to="/myCourses" class="cta-button">My Courses</router-link>
    </header>
      ...
      <ul class="course-list">
        <li v-for="course in filteredCourses" :key="course.id" class="course-card">
          ...
          <div class="course-actions">
            <button v-if="currrentUser ==='admin'|| currrentUser ==='student'" @click="enrollInCourse(course)" class="action-button enroll-button">
            ...
            <button v-if="currrentUser ==='admin'|| currrentUser ==='teacher' " @click="editCourse(course.id)" class="action-button edit-button">
...
<!--            </button>-->
            <button v-if="currrentUser ==='admin'" @click="showConfirmModal(course.id)" class="action-button delete-button">
              <i class="icon icon-delete"></i> Delete
            </button>
          </div>
        </li>
...
const currrentUser = ref('');
// Fetch the list of courses and the current user when the component is mounted
onMounted(() => {
  store.dispatch('fetchCourses');
  currrentUser.value = store.getters.currentUser.role;

  console.log(currrentUser.value,"tuka sme zimi se");
...
const filteredCourses = computed(() => {
  ...
  return courses.value.filter(course =>
      course.title.toLowerCase().startsWith(term)
  );
});
...
const enrollInCourse = async (course) => {
  try {
    console.log("spremame data")
    console.log(course.id);
    console.log(store.getters.currentUser)
    console.log(course.students)
    await store.dispatch('enrollInCourse', enrollData);
